...
#' nonfarm_employment
#' avg_weekly_hrs
#' avg_weekly_hrs_nonsup
#' nonfarm_productivity
#' nonfarm_labor_costs
#' nonfarm_hrly_compensation
#' nonfarm_multi_productivity
#' cpi_urban_80s
#' cpu_urban_67
#' core_cpi_u
#' core_cpi_w
...
#' ppi_final_ua
#' ppi_final_core
#' ppi_final_core_less_trade
#' ppi_finished_goods_82
#' imports_all_commodities
...
eco.download <- function(source, keys, shape, periodicity, show.recession, time.series){
  ...
  if(show.recession == 'y'){
    recession.name <- unlist(recession.indicators[periodicity], use.names = F)
...
# Shape dataset
  if(shape == 'wide'){
    ...
    if(show.recession == 'y'){names(df)[names(df) == paste0('value.',recession.name)] <- 'recession'}
